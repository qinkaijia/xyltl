...
- Dashboard Summary 会优先融合最近一次 `/api/evaluate/latest`，避免旧数据库样例覆盖当前 2K1000LA/2K0301 状态。
...
后台启动时，先在当前 PowerShell 设置环境变量，再直接启动 Python 进程：
...
SafeCloud 启动时会自动读取 `safecloud/.env` 中尚未出现在环境变量里的配置；命令下发响应会带 `transport_target`，用于确认 Web/API 实际连接的 MQTT Broker。
...
如果没有单独设置 `DOUBAO_VISION_API_KEY`，服务会回退读取 `DOUBAO_API_KEY`。真实密钥只写入本地未提交 `.env` 或运行环境，不要提交到 Git。
...
当前云端视觉请求使用火山方舟 `/api/v3/responses` 格式，发送 `input_image` 和 `input_text`。返回解析兼容 `output_text`、`output[].content[].text` 和旧 `choices[].message.content`，最后统一归一化为 `vision_status`。
