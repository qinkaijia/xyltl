...
- Web 大屏已内置 `/api/evaluate` 安全评估面板，可从浏览器选择 mock 场景并查看模型明细。
...
Web 大屏中的“安全评估”面板支持正常、温度预警、气体报警、振动报警、传感器离线五类模拟场景。当前它只向云端发送标准 JSON，不读取真实 2K0301 数据；后续真实采集接入后仍保持同一 `/api/evaluate` 请求格式。
