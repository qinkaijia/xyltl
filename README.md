...
- `runtime/vision/latest.jpg`：拍照检测关键帧，检测完成后 Qt 短暂展示，再恢复实时画面。
- `runtime/vision/vision_state.json`：最新视觉评估结果。
- `runtime/vision/mode_request.json`：Qt 写入的云端/本地/关闭模式请求。
- `runtime/vision/capture_request.json`：语音助手或 Qt “拍照检测”按钮写入的按需抓拍请求。
...
DISPLAY=:0 XAUTHORITY=/home/xylt/.Xauthority QT_QPA_PLATFORM=xcb \
  ...
  --vision-live-file /home/xylt/xylt/runtime/vision/live.jpg
```
...
新版 Qt HMI 采用分页布局：总览、环境监测、AI 分析、视觉、日志、语音助手独立页面。环境监测页集中显示温度、湿度、TVOC、eCO2、MQ-3、火焰、风险；视觉页默认显示 USB 摄像头实时画面，点击“拍照检测”后触发云端或本地模式检测，检测完成后短暂展示关键帧，再恢复实时画面；语音页在监听/思考/执行时显示三点动画。
